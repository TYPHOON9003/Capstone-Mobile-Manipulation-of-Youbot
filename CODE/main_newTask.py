...
def main():
    ...
    df1.to_csv('traj_newtask.csv', index=False, header=False)
    ...
    df.to_csv('config_newtask.csv',index=False,header=False)

    dfXerr = pd.DataFrame(Xerr)
    dfXerr.to_csv('Xerr_newtask.csv', index=False, header=False)
    ...
    plt.legend(loc="best")
    plt.grid()
    plt.savefig(f'Xerr_newtask,kp={kp_gain},ki={ki_gain}.png')
    plt.show()
