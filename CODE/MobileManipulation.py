...
def MobileManipulator(bot_para,config,joint_limits,Blist,M0e,Tb0,Tse_init, Tsc_init, Tsc_final, Tce_grasp, Tce_standoff,maxspeed,dt,\
                      Kp_gain,Ki_gain,k,traj_type,method):
...
#    df_traj.to_csv('traj.csv', index=False, header=False)
    ...
    while success==True:
        if count < len(traj)-1 :
            ...
            # run feedback
            Ve,Xerr,J_e = FeedbackControl(config, Xd, Xd_next, Kp, Ki, dt,e_int,Blist,M0e,Tb0,F6,joint_limits)

            # velocities
            Velocity_control = np.dot(np.linalg.pinv(J_e,1e-4),Ve)
